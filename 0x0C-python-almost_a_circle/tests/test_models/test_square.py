...
class Test_for_Square_basics(TestCase):
    """Test cases for the Square basics"""
    def setUp(self):
...
class Test_Square_update_with_kwargs(TestCase):
    ...
    def test_update_kwargs_id(self):
        s = Square(2, 1, 2, 42)
        s.update(id=95)
        self.assertEqual(str(s), "[Square] (95) 1/2 - 2")

    def test_update_kwargs_id_size(self):
        s = Square(2, 1, 2, 42)
        s.update(size=10, id=95)
        self.assertEqual(str(s), "[Square] (95) 1/2 - 10")

    def test_update_kwargs_id_size_x(self):
        s = Square(2, 1, 2, 42)
        s.update(size=10, id=95, x=30)
        self.assertEqual(str(s), "[Square] (95) 30/2 - 10")

    def test_update_kwargs_id_size_x_y(self):
        s = Square(2, 1, 2, 42)
        s.update(y=40, size=10, id=95, x=30)
        self.assertEqual(str(s), "[Square] (95) 30/40 - 10")

    def test_update_kwargs_too_many_args(self):
        s = Square(2, 1, 2, 42)
        s.update(y=40, size=10, id=95, x=30, betty="AlxSchool")
        self.assertEqual(str(s), "[Square] (95) 30/40 - 10")

    def test_update_kwargs_mixed_too_many_args(self):
        s = Square(2, 1, 2, 42)
        s.update(y=40, size=10, betty="AlxSchool", id=95, x=30, mub="2023")
        self.assertEqual(str(s), "[Square] (95) 30/40 - 10")

    def test_update_kwargs_args_before(self):
        s = Square(2, 1, 2, 95)
        s.update(42, 42, y=40, x=80)
        self.assertEqual(str(s), "[Square] (42) 1/2 - 42")

    def test_update_kwargs_not_all_mixed(self):
        s = Square(2, 1, 2, 95)
        s.update(y=40, x=40, size=40)
        self.assertEqual(str(s), "[Square] (95) 40/40 - 40")

    def test_update_kwargs_only_wrong_keys(self):
        s = Square(20, 10, 20, 95)
        s.update(betty="AlxSchool", AlxSchool="betty", mub="2023")
        self.assertEqual(str(s), "[Square] (95) 10/20 - 20")
